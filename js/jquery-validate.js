...
(function(factory) {
    ...
    var RULES = {
        isHttp:function(value,errorMsg){
            if(!/^([hH][tT]{2}[pP]:\/\/)(([A-Za-z0-9-~]+)\.)+([A-Za-z0-9-~\/])+$/.test(value)){
                return  errorMsg
            }
        },
    ...
    Validator.prototype.showMsg = function(target, msg, status, callback) {
        ...
        $msg.remove();
        $context.removeClass('success tip error').addClass(_current+' '+_other).append('<span class="valid_message" style="color:red;">' + msg + '</span>');
    };
